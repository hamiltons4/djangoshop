...
def index(request):
    ...
    return render(request, 'index.html', {'products':products, 'user':user, 'item':items, 'page':'home'})
...
def thank_you(request):
    ...
    if user.is_anonymous():
        ...
        items = cart_items.count() if cart_items else 0
    items_sum = calculate_sum(cart_items)
    return render(request, 'thankyou.html', {'user':user, 'items':items, 'page':'cart', 'cart_items':cart_items, 
    'sum':items_sum})
